...
def plot_filters_grid(pwms: list, output_plot_path: str, max_display: int = 16):
    ...
    fig, axes = plt.subplots(
        grid_size, grid_size, figsize=(grid_size * 3.5, grid_size * 2)
    )
    axes = axes.flatten()
    ...
    try:
        ...
        has_logomaker = False
        print(
            "Notice: logomaker library not installed. Falling back to heatmap visualization for filters."
        )
    ...
    for i in range(len(axes)):
        ...
        if i < n_display:
            ...
            if has_logomaker:
                ...
                ax.imshow(pwm, cmap="Blues", aspect="auto")
...
def save_pwms_to_meme(pwms: list, output_path: str, bg_freqs: dict = None):
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"A {bg_freqs['A']:.4f} C {bg_freqs['C']:.4f} G {bg_freqs['G']:.4f} T {bg_freqs['T']:.4f}\n\n"
        )
        ...
        for idx, pwm in enumerate(pwms):
            ...
            f.write(
                f"letter-probability matrix: alength= 4 w= {kernel_size} nsites= 100 E= 0\n"
            )

            # Print columns
            for col in range(kernel_size):
                f.write(
                    f"  {pwm[0, col]:.6f}  {pwm[1, col]:.6f}  {pwm[2, col]:.6f}  {pwm[3, col]:.6f}\n"
                )
            f.write("\n")


def compute_attribution(
    model, input_tensor, method: str = "saliency", target=None
) -> np.ndarray:
    """
    ...
    if has_captum:
        ...
        if method.lower() == "saliency":
            ...
            for step in range(1, steps + 1):
                alpha = step / steps
                x_interpolated = torch.tensor(
                    alpha * x_np, dtype=torch.float32, device=device, requires_grad=True
                )
...
def plot_sequence_attribution(
    ...
    sequence_onehot[:, start_pos:end_pos]
    ...
    if has_logomaker:
        ...
        im = ax.imshow(
            ...
            vmin=-np.max(np.abs(window_attr)),
            vmax=np.max(np.abs(window_attr)),
        )
        ax.set_yticks([0, 1, 2, 3])
    ...
    plt.title(
        f"Sequence Attribution Map (Positions {start_pos} - {end_pos})", fontsize=12
    )
    plt.tight_layout()
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
